...
document.addEventListener("DOMContentLoaded", function () {
  ...
  window.electron.onDeviceInfo((devices) => {
    if (deviceInfoList) {
      deviceInfoList.innerHTML = devices.map(device => `
        <div class="device-entry">
          <p><strong>Path:</strong> ${device.path}</p>
  ...
  window.electron.onActiveSensors((sensors) => {
    const list = document.getElementById("sensor-list");
    list.innerHTML = "";
    sensors.forEach(sensor => {
      const item = document.createElement("li");
      item.textContent = `Port ${sensor.port}: ${sensor.name} (${sensor.type})`;
